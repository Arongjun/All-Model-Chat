...
export const getModelIcon = (model: ModelOption | undefined) => {
    ...
    // Native Audio (Live)
    if (isNativeAudioModel) return <AudioWaveform size={MODEL_ICON_SIZE} className="text-amber-500 dark:text-amber-400 flex-shrink-0" strokeWidth={1.5} />;
    ...
    if (normalizedId.includes('gemini')) {
        return <Sparkles size={MODEL_ICON_SIZE} className="text-sky-500 dark:text-sky-400 flex-shrink-0" strokeWidth={1.5} />;
    }

    if (isPinned) return <Sparkles size={MODEL_ICON_SIZE} className="text-sky-500 dark:text-sky-400 flex-shrink-0" strokeWidth={1.5} />;
    return <Box size={MODEL_ICON_SIZE} className="text-[var(--theme-text-tertiary)] opacity-70 flex-shrink-0" strokeWidth={1.5} />;
};
                                                        ...
                                                        <div className="flex items-start gap-2.5 min-w-0 flex-grow overflow-hidden">
                                                            ...
                                                            <div className="min-w-0 flex-grow">
                                                                <div className="flex items-center gap-2 flex-wrap">
                                                                    <span className={`truncate ${isSelected ? 'text-[var(--theme-text-link)] font-semibold' : 'text-[var(--theme-text-primary)]'}`} title={entry.name}>
                                                                        {entry.name}
                                                                    </span>
                                                                </div>
