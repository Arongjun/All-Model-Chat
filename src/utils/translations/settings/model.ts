...
export const modelSettings = {
    ...
    settingsThinkingMode_tooltip: { en: 'Controls supported reasoning models. Gemini can use token budgets or levels; OpenAI-compatible reasoning models map levels to reasoning effort when supported by the upstream provider.', zh: '控制支持推理的模型。Gemini 可使用 Token 预算或推理等级；OpenAI-compatible 推理模型会在上游支持时把等级映射为 reasoning effort。' },
    settingsGemmaReasoningToggle_label: { en: 'Gemma Reasoning Level', zh: 'Gemma 推理等级' },
    settingsGemmaReasoningToggle_enabledDesc: { en: 'Gemma uses HIGH reasoning for better results, with higher latency.', zh: 'Gemma 使用 HIGH 推理以获得更好的结果，但延迟会更高。' },
    settingsGemmaReasoningToggle_disabledDesc: { en: 'Gemma uses MINIMAL reasoning for faster responses.', zh: 'Gemma 使用 MINIMAL 推理以获得更快的响应。' },
    ...
    settings_filesApi_desc: { en: 'Choose whether each file type prefers Files API uploads or inline Base64 delivery. Inline requests are limited to 100MB, or 50MB for PDFs; oversized files still fall back to Files API.', zh: '为每种文件类型选择优先通过 Files API 上传，还是尽量以内联 Base64 发送。内联请求上限为 100MB，PDF 为 50MB；超出限制时仍会回退到 Files API。' },
    ...
    settings_filesApi_tooltip: { en: 'On prefers Files API. Off sends content inline as Base64 when possible; oversized files may still use Files API.', zh: '打开时优先使用 Files API。关闭时会尽量以内联 Base64 发送；文件过大时仍可能使用 Files API。' },
    ...
    settingsSyncModels: { en: 'Sync API Models', zh: '同步模型' },
    settingsSyncingModels: { en: 'Syncing', zh: '同步中' },
    ...
    settingsGemmaReasoningTooltip: { en: 'Gemma 4 on the Gemini API uses thinkingConfig.thinkingLevel with two supported levels here: MINIMAL and HIGH.', zh: 'Gemma 4 在 Gemini API 中这里使用 thinkingConfig.thinkingLevel，两档分别是 MINIMAL 和 HIGH。' },
    // TTS Voice Styles
