...
export const appearanceSettings = {
    ...
    settings_autoFullscreenHtml_label: { en: 'Auto-open Preview', zh: '自动打开预览' },
    settings_autoFullscreenHtml_tooltip: { en: 'Automatically open a fullscreen preview after an HTML or SVG block is generated.', zh: '在 HTML 或 SVG 代码块生成后自动打开全屏预览。' },
    ...
    settings_autoSendOnSuggestionClick_label: { en: 'Tap to Send Reply', zh: '点按回复即发' },
    settings_autoSendOnSuggestionClick_tooltip: { en: 'When enabled, clicking a follow-up reply suggestion will send it immediately. Other suggestion types will only populate the input field.', zh: '启用后，点击跟进型回复建议将立即发送。其他类型的建议只会填充输入框。' },
    ...
    settings_rawMode_tooltip: { en: 'For supported models, pre-fill the response with <thinking> so replies start in raw reasoning mode. Unsupported models ignore this setting.', zh: '对支持的模型，会在回复前预填 <thinking>，让输出从原始推理模式开始。不支持的模型会忽略该设置。' },
